...
| --- | --- | --- |
| `old-tom.jpg` | Clean bourbon label, all required fields, canonical Government Warning in bold all-caps | **Pass** on every field |
| `altered-warning.jpg` | Same as old-tom, but the warning header is in title case | **Fail** on the Government Warning, pass on everything else |
| `chateau-margaux.jpg` | Wine label with country of origin | **Pass** on every field (including the optional country-of-origin check and the beverage-type classification) |

`samples/applications.csv` pairs each image with its application data including the `beverage_class` column.
...
The tests pin every comparison rule — including the regulatory edge cases each stakeholder called out — so a future change can't silently break a compliance behaviour.
