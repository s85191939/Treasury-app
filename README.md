...
| **Vision model via OpenRouter** | One API key, easy to swap models without touching code. Defaults to `google/gemini-2.5-flash` — measured at ~2.4s end-to-end per label in local testing, comfortably inside Sarah's 5s budget, with reliable OCR and tool/function calling. `OPENROUTER_MODEL` swaps in `anthropic/claude-haiku-4.5` (~3.5s, very accurate), `anthropic/claude-sonnet-4.5` (~5-6s, highest quality), or `openai/gpt-4o` without code changes. |
