...
4. **The regulatory decision is deterministic code, not the model.** The model only extracts text from the image. A pure-TypeScript comparator in `src/lib/compare.ts` applies the rules, and **73 unit tests** + **4 end-to-end browser tests** pin every rule so a future change can't silently break a compliance behaviour.
...
| `riverstone-ai.jpg` | **AI-generated** label (Gemini 2.5 Flash Image via OpenRouter) | Real-world test on an AI-generated image — the generator produced typos in the warning text (`GOVERIMENT WARNING:`, `alconic beverages`) which the verifier catches as a fail, exactly the kind of regulatory miss an agent would spot. The spec encouraged generating AI labels for testing; this one demonstrates the verifier handling them. |
...
```bash
npm test                # 73 unit tests, ~180ms
npm run test:watch      # vitest watch mode
npm run test:e2e        # 4 playwright tests against the live deployment, ~20s
...
Pure-function tests on the comparator, the OpenRouter extractor (with `fetch` mocked), and the CSV parser. 73 tests, sub-200ms.
...
| --- | --- | --- |
| `src/lib/compare.test.ts` | 55 | every comparison rule: similarity boundaries, ABV/volume tolerance, beverage-type ABV exemptions (14% wine boundary, beer N/A), STONE'S THROW judgment, Government Warning (caps + bold + canonical text + near-miss handling), class/type fuzzy edges, country-of-origin permutations, overallStatus aggregation |
...
Playwright drives a real headless Chromium against the **live Vercel deployment** — exactly the way a reviewer would test it. The scripts fill the form, upload sample images, click *Verify*, and assert on the rendered DOM. 4 tests, ~20s.
...
Running 4 tests using 1 worker
  ...
  ✓  1 single label — happy path (OLD TOM DISTILLERY passes) (3.9s)
  ✓  2 single label — strict Government Warning fail (title case) (3.2s)
  ✓  3 single label — wine-under-14% ABV exemption (3.5s)
  ✓  4 batch mode — CSV + image folder (8.4s)
