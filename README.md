...
- [x] Unit tests — 80 tests across [`compare.test.ts`](src/lib/compare.test.ts), [`csv.test.ts`](src/lib/csv.test.ts), [`extract.test.ts`](src/lib/extract.test.ts); 5 e2e tests in [`e2e/live.spec.ts`](e2e/live.spec.ts). `npm run test:all`
...
For a full multi-rule failure on one label, upload [`samples/multi-violation.jpg`](samples/multi-violation.jpg) against the pre-filled OLD TOM application but change Net Contents to `800 mL`. Seven distinct rules fire in one verdict: brand, class, ABV, net contents, Standards of Fill, producer, and Government Warning case.

To exercise batch mode:

1. Click **Batch (CSV + images)**.
2. Upload [`samples/applications.csv`](samples/applications.csv) (or build your own — see the column list above).
3. Upload the matching images from `samples/`. They pair by filename.
4. Hit **Verify all** — the table fills in live with pass / review / fail. Click **Export results CSV** to take the verdicts back to COLA.

---

## Included test fixtures

The `samples/` folder ships **23 test fixtures** across three sources:

- **9 deterministic synthetic labels** under `samples/*.jpg`, generated by [`samples/generate.py`](samples/generate.py). Each one exercises a specific rule (pass scenarios, strict-warning fails, ABV exemption boundaries, a kitchen-sink fixture that breaks every rule simultaneously).
...
| `multi-violation.jpg` | "SCRUFFY MOON" Vodka label paired with an OLD TOM bourbon application at 800 mL | **Fail** | Kitchen-sink: brand mismatch, class mismatch (Vodka vs Bourbon), ABV mismatch, net-contents mismatch, Standards of Fill (800 mL not approved), producer mismatch, Government Warning case fail — seven rules fire in one verdict. |
...
└─ lib/
   ...
   ├─ compare.test.ts         # 62 vitest unit tests
...
Pure-function tests on the comparator, the OpenRouter extractor (with `fetch` mocked), and the CSV parser. **80 tests**, sub-200ms.
...
**The regression test that exists because of a real bug.** While building a multi-rule failure fixture, the comparator returned `85%` similarity for `Vodka` vs `Kentucky Straight Bourbon Whiskey`. Root cause: the in-place Levenshtein DP swapped `prev[0]↔curr[0]` before copying `curr → prev` each iteration, which clobbered the row index and put very different strings of mismatched length into the Review band instead of Fail. Fixed; the regression test `"rates very different strings of unequal length as near-zero similarity"` would have caught it. The deterministic comparator design is exactly what makes a bug like this discoverable.
